...
export default function Editor({
  ...
  const addReplyToComment = (commentId: string, replyContent: string) => {
    ...
    function findMarkPositions(state: EditorState, id: string) {
      const positions: { from: number; to: number }[] = [];

      // Traverse the document
      state.doc.descendants((node, pos) => {
        // console.log("Node", node);
        // Check if the node is a text node with the desired mark
        node.marks.forEach((mark) => {
          // console.log("Mark", mark, mark.attrs);
          if (mark.attrs.commentId === id) {
            // We've found a mark with the desired attribute
            // Assuming the mark covers the whole text node (adjust if needed)
            positions.push({ from: pos, to: pos + node.text.length });
          }
        });
    ...
    const markPos = findMarkPositions(editor!.state, commentId);
    console.log("Mark pos", markPos);

    if (markPos[0]) {
      headlessEditor.commands.setTextSelection(markPos[0]);
      headlessEditor.commands.deleteSelection();
    }
  ...
  useEffect(() => {
    ...
    headlessEditor?.commands.insertContent(diff);
